...
public class HorizontalProgressView extends ProgressBar {
    ...
    private int mNormalBarSize = dp2px(2);//Utils.dp2px(getContext(), 2);
    private int mNormalBarColor = Color.parseColor("#FFD3D6DA");
    private int mReachBarSize = dp2px(2);//Utils.dp2px(getContext(), 2);
    private int mReachBarColor = Color.parseColor("#108ee9");
    private int mTextSize = sp2px(14);//Utils.sp2px(getContext(), 14);
    private int mTextColor = Color.parseColor("#108ee9");
    private int mTextOffset = dp2px(6);//Utils.dp2px(getContext(), 6);
    ...
    private void drawHorizontalProgressView(Canvas canvas) {
        ...
        float progressPosX = mDrawWidth * radio;

        // 绘制背景
        //canvas.drawRoundRect(0, 0, mDrawWidth, mNormalBarSize, 50, 50, mNormalPaint);最低API要求21
        canvas.drawRoundRect(new RectF(0, 0, mDrawWidth, mNormalBarSize), 10, 10, mNormalPaint);
        // 绘制进度
        float top = mNormalBarSize - mReachBarSize;
        float endX = progressPosX - top;
        if (endX > 0) {
            //方法：drawRoundRect( 0 , 0, width, height, mRadius, mRadius, mPaint) 的确是api21的
            //不过可以用这个方法代替：drawRoundRect(new RectF(0, 0, width, height), mRadius, mRadius, mPaint)
            //canvas.drawRoundRect(top, top, endX, mReachBarSize, 90, 90, mReachPaint);最低API要求21
            canvas.drawRoundRect(new RectF(top, top, endX, mReachBarSize), 10, 10, mReachPaint);
...
//        canvas.drawText(text, progressPosX, -textHeight, mTextPaint);
                canvas.drawText(text, (endX - textWidth - mTextOffset), mNormalBarSize / 2 - textHeight, mTextPaint);
                break;
        }
    }
